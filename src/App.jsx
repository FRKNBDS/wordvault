...
function AuthScreen({ onLogin }) {
  ...
  const adminLogin = async () => {
    // Şifreyi Supabase'deki settings tablosundan çek
    const { data, error } = await supabase.from("settings").select("value").eq("key","adminPassword").single();
    const correctPass = data?.value ?? "admin"; // default: admin
    if (adminPass !== correctPass) { setError(lang==="en"?"Incorrect password.":"Şifre hatalı."); return; }
...
function SettingsScreen({ user, lang, onLangChange, onUserUpdate, onLogout, onSwitchUser, showToast }) {
  ...
  const saveAdminPw = async () => {
    ...
    const { data: settingRow } = await supabase.from("settings").select("value").eq("key","adminPassword").single();
    const currentPass = settingRow?.value ?? "admin";
    if (adminOld !== currentPass){setPwErr(lang==="en"?"Old password incorrect.":"Eski şifre hatalı.");return;}
    ...
    // Supabase'e kaydet — tüm cihazlara anında yansır
    const { error } = await supabase.from("settings").upsert([{key:"adminPassword", value:adminNew}], {onConflict:"key"});
...
function AdminScreen({ user, lang, showToast }) {
  ...
  if(selSet) loadWords(selSet.id);
          ...
          {!selLevel && <>
            ...
            {levels.map(level=>(
              <div key={level.id} className="admin-item" onClick={()=>{setSelLevel(level);loadSets(level.id);}}>
                ...
                <button className="del-btn" onClick={async e=>{e.stopPropagation();await supabase.from("levels").delete().eq("id",level.id);loadLevels();showToast(lang==="en"?"Deleted":"Silindi");}}>🗑</button>
              </div>
            ))}
          </>}
          ...
          {selLevel&&!selSet&&<>
            ...
            {sets.map(set=>(
              <div key={set.id} className="admin-item" onClick={()=>{setSelSet(set);loadWords(set.id);}}>
                ...
                <span style={{color:"var(--w4)",fontSize:17}}>›</span>
                <button className="del-btn" onClick={async e=>{e.stopPropagation();await supabase.from("wordsets").delete().eq("id",set.id);loadSets(selLevel.id);showToast(lang==="en"?"Deleted":"Silindi");}}>🗑</button>
              </div>
            ))}
          </>}
          ...
          {selSet&&<>
            ...
            {words.map(word=>(
              <div key={word.id} className="word-item" style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                ...
                <button style={{background:"none",border:"none",color:"var(--g2)",fontSize:15,cursor:"pointer",padding:"2px 5px"}} onClick={()=>{setEditWord(word);setModal("editWord");}}>✏️</button>
                <button className="del-btn" onClick={async()=>{await supabase.from("words").delete().eq("id",word.id);loadWords(selSet.id);}}>🗑</button>
              </div>
            ))}
          </>}
        </>}
      ...
      {/* MODALS */}
      {modal==="level"&&<CreateLevelModal lang={lang} onClose={()=>setModal(null)} onSave={async d=>{await supabase.from("levels").insert([{...d,orderIndex:levels.length,createdAt:Date.now()}]);loadLevels();setModal(null);showToast(lang==="en"?"Level created!":"Seviye oluşturuldu!");}}/>}
      {modal==="set"&&selLevel&&<CreateSetModal lang={lang} levelName={selLevel.name} onClose={()=>setModal(null)} onSave={async d=>{await supabase.from("wordsets").insert([{...d,levelId:selLevel.id,orderIndex:sets.length,createdAt:Date.now()}]);loadSets(selLevel.id);setModal(null);showToast(lang==="en"?"Set created!":"Set oluşturuldu!");}}/>}
...
function EditWordModal({lang,word,onClose,onSaved}){
  ...
  const save = async () => {
    await supabase.from("words").update({...f}).eq("id", word.id);
    onSaved();
  };
...
function FullscreenBtn({lang}) {
  const [fs, setFs] = React.useState(!!document.fullscreenElement);
  React.useEffect(() => {
  ...
  return (
    <div className="srow" onClick={toggle} style={{cursor:"pointer"}}>
      ...
      <span style={{marginLeft:"auto",fontSize:20,opacity:.6}}>{fs?"🔲":"⛶"}</span>
    </div>
  );
}
...
function ImportModal({lang,set,onClose,onDone,showToast}) {
  ...
  const doImport = async (text, isJson) => {
    ...
    try {
      ...
      const { error } = await supabase.from("words").insert(validRows);
