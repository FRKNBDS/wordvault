...
async function getStudyWords(userId, dailyGoal, sessionType) {
  ...
  const { data: _allW } = await supabase.from("words").select("*").limit(10000);
...
async function getPracticeWords(userId, setIds) {
  const { data } = await supabase.from("words").select("*").in("wordSetId", setIds).limit(10000);
...
function ExploreScreen({ user, lang, showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    // 2) Arka planda Supabase'den her zaman sync yap
    try {
