...
function AuthScreen({ onLogin }) {
  ...
  const adminLogin = async () => {
    const { data } = await supabaseAdmin.from("settings").select("value").eq("key","adminPassword").single();
    const correctHash = data?.value ?? await hashPassword("admin");
    const inputHash   = await hashPassword(adminPass);
    if (inputHash !== correctHash) { setError(lang==="en"?"Incorrect password.":"Şifre hatalı."); return; }
...
function SettingsScreen({ user, lang, onLangChange, onUserUpdate, onLogout, onSwitchUser, showToast }) {
  ...
  const saveAdminPw = async () => {
    setPwErr("");
    // Mevcut şifreyi Supabase'den doğrula
    const { data: settingRow } = await supabaseAdmin.from("settings").select("value").eq("key","adminPassword").single();
    const currentHash = settingRow?.value ?? await hashPassword("admin");
    const oldHash     = await hashPassword(adminOld);
    if (oldHash !== currentHash){setPwErr(lang==="en"?"Old password incorrect.":"Eski şifre hatalı.");return;}
    ...
    const newHash = await hashPassword(adminNew);
    const { error } = await supabaseAdmin.from("settings").upsert([{key:"adminPassword", value:newHash}], {onConflict:"key"});
    if (error) { setPwErr("Kayıt hatası: " + error.message); return; }
...
function AdminScreen({ user, lang, showToast }) {
  ...
  await supabaseAdmin.from("words").delete().eq("id", word.id);
  loadAllWords();
  if(selSet) loadWords(selSet.id);
          ...
          {!selLevel && <>
            ...
            {levels.map(level=>(
              <div key={level.id} className="admin-item" onClick={()=>{setSelLevel(level);loadSets(level.id);}}>
                ...
                <button className="del-btn" onClick={async e=>{e.stopPropagation();await supabaseAdmin.from("levels").delete().eq("id",level.id);loadLevels();showToast(lang==="en"?"Deleted":"Silindi");}}>🗑</button>
              </div>
            ))}
          </>}
          ...
          {selLevel&&!selSet&&<>
            ...
            {sets.map(set=>(
              <div key={set.id} className="admin-item" onClick={()=>{setSelSet(set);loadWords(set.id);}}>
                ...
                <span style={{color:"var(--w4)",fontSize:17}}>›</span>
                <button className="del-btn" onClick={async e=>{e.stopPropagation();await supabaseAdmin.from("wordsets").delete().eq("id",set.id);loadSets(selLevel.id);showToast(lang==="en"?"Deleted":"Silindi");}}>🗑</button>
              </div>
            ))}
          </>}
          ...
          {selSet&&<>
            ...
            {words.map(word=>(
              <div key={word.id} className="word-item" style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                ...
                <button style={{background:"none",border:"none",color:"var(--g2)",fontSize:15,cursor:"pointer",padding:"2px 5px"}} onClick={()=>{setEditWord(word);setModal("editWord");}}>✏️</button>
                <button className="del-btn" onClick={async()=>{await supabaseAdmin.from("words").delete().eq("id",word.id);loadWords(selSet.id);}}>🗑</button>
              </div>
            ))}
          </>}
        </>}
      ...
      {/* MODALS */}
      {modal==="level"&&<CreateLevelModal lang={lang} onClose={()=>setModal(null)} onSave={async d=>{await supabaseAdmin.from("levels").insert([{...d,orderIndex:levels.length,createdAt:Date.now()}]);loadLevels();setModal(null);showToast(lang==="en"?"Level created!":"Seviye oluşturuldu!");}}/>}
      {modal==="set"&&selLevel&&<CreateSetModal lang={lang} levelName={selLevel.name} onClose={()=>setModal(null)} onSave={async d=>{await supabaseAdmin.from("wordsets").insert([{...d,levelId:selLevel.id,orderIndex:sets.length,createdAt:Date.now()}]);loadSets(selLevel.id);setModal(null);showToast(lang==="en"?"Set created!":"Set oluşturuldu!");}}/>}
...
function AddWordModal({lang,setId,orderIndex,onClose,onSaved,checkDuplicate}){
  ...
  const save=async()=>{
    ...
    if (insertErr) { setErr("Kayıt hatası: " + insertErr.message); return; }
    onSaved();
...
function EditWordModal({lang,word,onClose,onSaved}){
  ...
  const save = async () => {
    await supabaseAdmin.from("words").update({...f}).eq("id", word.id);
    onSaved();
  };
...
function ImportModal({lang,set,onClose,onDone,showToast}) {
  ...
  const doImport = async (text, isJson) => {
    ...
    try {
      ...
      const { error } = await supabaseAdmin.from("words").insert(validRows);
