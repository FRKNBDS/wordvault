...
.ob-overlay{position:fixed;inset:0;z-index:99999;background:linear-gradient(160deg,#0a0f1a,#0d1526);display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px 28px 24px}
.ob-slides{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;max-width:400px;text-align:center}
.ob-icon{font-size:72px;margin-bottom:24px;animation:bob 2s ease-in-out infinite}
.ob-title{font-family:var(--fd);font-size:26px;color:var(--g1);font-weight:700;letter-spacing:1px;margin-bottom:16px}
.ob-desc{font-size:15px;color:var(--w6);line-height:1.7;margin-bottom:32px}
.ob-dots{display:flex;gap:8px;margin-bottom:32px}
.ob-dot{width:8px;height:8px;border-radius:50%;background:var(--w2);transition:all .3s}
.ob-dot.active{width:24px;border-radius:4px;background:var(--g1)}
.ob-btn{width:100%;max-width:280px;padding:14px;background:linear-gradient(135deg,var(--g1),var(--g2));border-radius:14px;border:none;color:#000;font-family:var(--fd);font-size:15px;font-weight:700;letter-spacing:1px;cursor:pointer;margin-bottom:20px}
.ob-skip{font-size:13px;color:var(--w3);cursor:pointer;text-decoration:underline;background:none;border:none;padding:4px}
.ob-check{display:flex;align-items:center;gap:10px;margin-top:8px;cursor:pointer}
.ob-check input{width:18px;height:18px;accent-color:var(--g1);cursor:pointer}
.ob-check span{font-size:13px;color:var(--w4)}
`;
...
const OB_SLIDES = [
  {
    icon: "⚓",
    title: "Wordvault'a Hoş Geldin",
    desc: "YDS kelimelerini akıllı kartlar ve tekrar sistemiyle kalıcı olarak öğren. Günde birkaç dakika yeterli."
  },
  {
    icon: "🗂",
    title: "Nasıl Çalışır?",
    desc: "Kelimeler 5 kutuda ilerler. Bildiğin kelimeler ileriye geçer, bilmediklerin geri döner. Böylece sadece zorlandığın kelimeleri tekrarlarsın."
  },
  {
    icon: "🏆",
    title: "Pratik & Tekrar",
    desc: "Günlük hedefini belirle, çalışma oturumlarını tamamla ve istatistiklerinle ilerlemeyi takip et. Başarı seni bekliyor!"
  }
];
...
function OnboardingScreen({ onDone, userId }) {
  ...
  const finish = (skip) => {
    if (noMore || skip) localStorage.setItem("wv_ob_done_" + userId, "1");
    onDone();
  };
  ...
  return (
    <div className="ob-overlay">
      <div className="ob-slides">
        ...
        {!isLast && (
          <button className="ob-skip" onClick={skip}>Atla</button>
        )}
        {isLast && (
