...
async function getStudyWords(userId, dailyGoal, sessionType) {
  ...
  for (const wid of wordIds) {
    const w = await dGet("words", wid);
    if (w) wordById[w.id] = w;
  }
...
async function getPracticeWords(userId, setIds) {
  const allWords = await dAll("words");
  return allWords.filter(w => setIds.includes(w.wordSetId)).sort(()=>Math.random()-0.5);
}
...
async function deactivateSet(userId, wordSetId) {
  ...
  const otherActiveWords = new Set(
    allWords.filter(w => otherActiveSetIds.includes(w.wordSetId)).map(w => w.id)
  );
  for (const w of words) {
...
// ── OFFLINE-FIRST SYNC ────────────────────────────────────────────────────────
// İlk açılışta tüm veriyi Supabase'den çekip IndexedDB'ye kaydeder.
// Sonraki açılışlarda IndexedDB'den okur (internet gerekmez).
// Realtime ile admin değişikliklerini anında IndexedDB'ye yazar.

async function initialSync() {
  const already = localStorage.getItem("wv_synced");
  if (!already) {
    await fullSync(); // İlk kez: tüm veriyi bekleyerek çek
    localStorage.setItem("wv_synced", "1");
  } else {
    fullSync(); // Sonraki açılışlar: arka planda güncelle, UI'ı bloklama
  }
  startRealtime();
}
...
async function fullSync() {
  try {
    const [lvlRes, wsRes, wRes] = await Promise.all([
      supabase.from("levels").select("*"),
      supabase.from("wordsets").select("*"),
      supabase.from("words").select("*"),
    ]);
    const db = await openDB();
    await new Promise((res, rej) => {
      const tx = db.transaction(["levels","wordsets","words"], "readwrite");
      tx.onerror = () => rej(tx.error);
      tx.oncomplete = () => res();
      const lStore = tx.objectStore("levels");
      const wsStore = tx.objectStore("wordsets");
      const wStore  = tx.objectStore("words");
      (lvlRes.data  || []).forEach(r => lStore.put(r));
      (wsRes.data   || []).forEach(r => wsStore.put(r));
      (wRes.data    || []).forEach(r => wStore.put(r));
    });
    localStorage.setItem("wv_last_sync", Date.now().toString());
  } catch(e) {
    console.warn("fullSync hatası:", e);
  }
}

function startRealtime() {
  // Zaten başlatılmışsa tekrar başlatma
  if (window.__wv_realtime__) return;
  window.__wv_realtime__ = true;

  const handleChange = async (table, payload) => {
    try {
      if (payload.eventType === "INSERT" || payload.eventType === "UPDATE") {
        await dPut(table, payload.new);
      } else if (payload.eventType === "DELETE") {
        await dDel(table, payload.old.id);
      }
      // Sayfayı yenile sinyali gönder
      window.dispatchEvent(new CustomEvent("wv_db_updated", { detail: { table } }));
    } catch(e) {
      console.warn("Realtime sync hatası:", e);
    }
  };

  supabase.channel("wv_sync")
    .on("postgres_changes", { event: "*", schema: "public", table: "words" },
      p => handleChange("words", p))
    .on("postgres_changes", { event: "*", schema: "public", table: "wordsets" },
      p => handleChange("wordsets", p))
    .on("postgres_changes", { event: "*", schema: "public", table: "levels" },
      p => handleChange("levels", p))
    .subscribe();
}
  ...
  useEffect(() => {
    const boot = async () => {
      try {
        ...
        // Offline-first sync: ilk açılışta tümünü çek, sonra realtime başlat
        await initialSync();
...
function ExploreScreen({ user, lang, showToast }) {
  ...
  const load = useCallback(async () => {
    const [lvls, sets, acts, words] = await Promise.all([
      dAll("levels"),
      dAll("wordsets"),
      dIdx("active_sets", "userId", user.id),
      dAll("words"),
    ]);
    const sortedLvls = lvls.sort((a,b)=>a.orderIndex-b.orderIndex);
    setLevels(sortedLvls);
  ...
  // Realtime güncellemelerini dinle
  useEffect(()=>{
    const handler = () => load();
    window.addEventListener("wv_db_updated", handler);
    return () => window.removeEventListener("wv_db_updated", handler);
  },[load]);
...
function AdminScreen({ user, lang, showToast }) {
  ...
  const loadLevels = async () => {
    const data = await dAll("levels");
    setLevels(data.sort((a,b)=>a.orderIndex-b.orderIndex));
  };
  const loadSets = async (lid) => {
    const data = await dAll("wordsets");
    setSets(data.filter(s=>s.levelId===lid).sort((a,b)=>a.orderIndex-b.orderIndex));
  };
  const loadWords  = async (sid) => {
    const data = await dAll("words");
    setWords(data.filter(w=>w.wordSetId===sid).sort((a,b)=>a.orderIndex-b.orderIndex));
  };
  ...
  // Realtime güncellemelerini dinle (admin paneli)
  useEffect(()=>{
    const handler = (e) => {
      loadAllWords();
      if (selLevel) loadSets(selLevel.id);
      if (selSet) loadWords(selSet.id);
      if (e.detail?.table === "levels") loadLevels();
    };
    window.addEventListener("wv_db_updated", handler);
    return () => window.removeEventListener("wv_db_updated", handler);
  },[selLevel, selSet]);
  ...
  if(selSet) loadWords(selSet.id);
      ...
      {modal==="word"&&selSet&&<AddWordModal lang={lang} setId={selSet.id} orderIndex={words.length} onClose={()=>setModal(null)} onSaved={()=>{loadWords(selSet.id);loadAllWords();showToast(lang==="en"?"Word added!":"Kelime eklendi!");}} checkDuplicate={async(eng)=>{const allW=await dAll("words");return allW.filter(w=>w.wordSetId===selSet.id).some(w=>w.english.toLowerCase()===eng.toLowerCase());}}/>}
