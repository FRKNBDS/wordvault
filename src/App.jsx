...
// ── ŞİFRE HASH (SHA-256, Web Crypto API) ─────────────────────────────────────
...
async function fetchWordData(allowNetwork) {
  if (!allowNetwork) {
    // Gizlilik politikası henüz onaylanmadı: hiçbir ağ isteği yapılmaz,
    // sadece gömülü (cihaz içi) veriyle devam edilir.
...
const dClear = (s)       => dbTx(s,"readwrite", (tx,r)=>{ const q=tx.objectStore(s).clear();    q.onsuccess=()=>r(); });

// Supabase'den gelen güncel içeriği (levels/wordsets/words) IndexedDB'ye yazar.
// Böylece bir sonraki açılışta internet olmasa da en güncel veri cihazda kalır.
async function syncContentToDB(lvls, sets, words) {
  await Promise.all([
    ...(lvls  || []).map(l => dPut("levels",   l)),
    ...(sets  || []).map(s => dPut("wordsets", s)),
    ...(words || []).map(w => dPut("words",    w)),
  ]);
}
...
async function initDB(wordData) {
  ...
  if (!users.find(u => u.username === "Admin")) {
    await dAdd("users", {
      username: "Admin", passwordHash: await hashPassword("admin"),
...
async function getStudyWords(userId, dailyGoal, sessionType) {
  ...
  // Kelimeler IndexedDB'den (cihazda) okunur — internet gerekmez.
  // Explore ekranı veya arka plan senkronizasyonu IndexedDB'yi güncel tutar.
  ...
  const wordIds = result.map(p=>p.wordId);
  const wordRows = await Promise.all(wordIds.map(id=>dGet("words", id)));
  const wordById = {}; (wordRows||[]).forEach(w=>{ if (w) wordById[w.id]=w; });
  return result.map(p=>({word:wordById[p.wordId], progress:p})).filter(x=>x.word);
}
...
async function getPracticeWords(userId, setIds) {
  const allWords = await dAll("words");
  const data = allWords.filter(w => setIds.includes(w.wordSetId));
...
async function activateSet(userId, wordSetId) {
  ...
  const allWords = await dAll("words");
  const words = allWords.filter(w => w.wordSetId === wordSetId);
...
async function deactivateSet(userId, wordSetId) {
  ...
  if (otherActiveSetIds.length) {
    allWords.filter(w => otherActiveSetIds.includes(w.wordSetId)).forEach(w=>otherActiveWords.add(w.id));
  }
  for (const w of words) {
...
.app{position:fixed;top:0;left:50%;transform:translateX(-50%);width:100%;max-width:500px;height:100%;display:flex;flex-direction:column;background:linear-gradient(160deg,var(--n1),var(--n0));overflow:hidden}@media(min-width:768px){.app{max-width:600px;font-size:18px}}@media(min-width:1024px){.app{max-width:680px}}
...
/* AUTH */
.auth-bg{height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 28px 40px;background:linear-gradient(160deg,var(--n1),var(--n0));position:relative;overflow:hidden}
...
.fs-btn{position:fixed;top:12px;right:12px;z-index:9999;background:rgba(0,0,0,0.4);border:1px solid rgba(212,175,55,0.3);border-radius:8px;color:var(--g2);font-size:16px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;backdrop-filter:blur(4px);transition:all .2s}
.fs-btn:active{background:rgba(212,175,55,0.2)}
.ob-overlay{position:fixed;inset:0;z-index:99999;background:linear-gradient(160deg,#080d18,#0d1526);display:flex;flex-direction:column;overflow:hidden}
.ob-header{padding:20px 24px 0;display:flex;justify-content:space-between;align-items:center;flex-shrink:0}
.ob-logo{font-family:var(--fd);font-size:15px;color:var(--g1);letter-spacing:2px}
.ob-step-info{font-size:12px;color:var(--w3)}
.ob-body{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:16px 24px;min-height:0}
.ob-sim{width:100%;max-width:320px;background:linear-gradient(160deg,#0f1824,#111d2e);border-radius:20px;border:1px solid rgba(255,255,255,0.08);overflow:hidden;position:relative;margin-bottom:24px;flex-shrink:0}
.ob-sim-header{background:rgba(212,175,55,0.08);padding:10px 14px;display:flex;align-items:center;gap:8px;border-bottom:1px solid rgba(255,255,255,0.05)}
.ob-sim-dot{width:7px;height:7px;border-radius:50%}
.ob-sim-title{font-family:var(--fd);font-size:11px;color:var(--w4);letter-spacing:1px}
.ob-sim-body{padding:14px}
.ob-sim-tabs{display:flex;gap:4px;margin-bottom:12px}
.ob-sim-tab{flex:1;padding:6px 4px;border-radius:8px;background:var(--w05);text-align:center;font-size:9px;color:var(--w3);transition:all .4s}
.ob-sim-tab.active{background:rgba(212,175,55,0.15);color:var(--g1);font-weight:700}
.ob-sim-card{background:rgba(212,175,55,0.06);border:1px solid rgba(212,175,55,0.15);border-radius:12px;padding:14px;text-align:center;margin-bottom:8px}
.ob-sim-word{font-family:var(--fd);font-size:18px;color:var(--w);margin-bottom:4px}
.ob-sim-tr{font-size:12px;color:var(--g2)}
.ob-sim-hint{font-size:10px;color:var(--w3);text-align:center;margin-bottom:10px}
.ob-sim-btns{display:flex;gap:6px}
.ob-sim-btn-no{flex:1;padding:8px;border-radius:8px;background:rgba(229,57,53,0.15);border:1px solid rgba(229,57,53,0.3);color:#ef5350;font-size:10px;text-align:center}
.ob-sim-btn-yes{flex:1;padding:8px;border-radius:8px;background:rgba(80,200,120,0.15);border:1px solid rgba(80,200,120,0.3);color:#66bb6a;font-size:10px;text-align:center}
.ob-sim-sets{display:flex;flex-direction:column;gap:6px}
.ob-sim-set{padding:10px 12px;border-radius:10px;background:var(--w05);display:flex;align-items:center;gap:8px;transition:all .4s}
.ob-sim-set.highlight{background:rgba(212,175,55,0.12);border:1px solid rgba(212,175,55,0.3)}
.ob-sim-set-icon{font-size:16px}
.ob-sim-set-name{font-size:11px;color:var(--w6);font-weight:600}
.ob-sim-set-count{font-size:10px;color:var(--w3)}
.ob-sim-start-btn{width:100%;padding:10px;border-radius:10px;background:linear-gradient(135deg,var(--g1),var(--g2));text-align:center;font-family:var(--fd);font-size:11px;color:#000;font-weight:700;margin-bottom:10px;transition:all .4s}
.ob-sim-start-btn.pulse{box-shadow:0 0 0 4px rgba(212,175,55,0.3);transform:scale(1.02)}
.ob-sim-boxes{display:flex;gap:4px;justify-content:center;margin-top:4px}
.ob-sim-box{width:44px;height:36px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:8px;color:var(--w3);gap:2px;transition:all .6s}
.ob-sim-box.filled{border-color:rgba(212,175,55,0.4);background:rgba(212,175,55,0.08)}
.ob-sim-box-num{font-size:14px}
.ob-sim-arrow{font-size:18px;color:var(--g1);text-align:center;animation:pulse 1s ease-in-out infinite}
@keyframes pulse{0%,100%{opacity:0.4;transform:scale(0.9)}50%{opacity:1;transform:scale(1.1)}}
.ob-arrow-hint{position:absolute;pointer-events:none;transition:all .5s}
.ob-caption{text-align:center;margin-bottom:4px}
.ob-caption-title{font-family:var(--fd);font-size:16px;color:var(--w);font-weight:700;margin-bottom:6px}
.ob-caption-desc{font-size:13px;color:var(--w5);line-height:1.6}
.ob-footer{padding:16px 24px 28px;flex-shrink:0}
.ob-dots{display:flex;gap:6px;justify-content:center;margin-bottom:14px}
.ob-dot{width:6px;height:6px;border-radius:50%;background:var(--w2);transition:all .3s}
.ob-dot.active{width:20px;border-radius:3px;background:var(--g1)}
.ob-btn{width:100%;padding:13px;background:linear-gradient(135deg,var(--g1),var(--g2));border-radius:12px;border:none;color:#000;font-family:var(--fd);font-size:14px;font-weight:700;letter-spacing:1px;cursor:pointer;margin-bottom:10px}
.ob-bottom{display:flex;align-items:center;justify-content:space-between}
.ob-skip{font-size:12px;color:var(--w3);cursor:pointer;background:none;border:none;padding:4px;text-decoration:underline}
.ob-check{display:flex;align-items:center;gap:8px;cursor:pointer}
.ob-check input{width:16px;height:16px;accent-color:var(--g1);cursor:pointer}
.ob-check span{font-size:12px;color:var(--w4)}
`;


// ── ONBOARDING ────────────────────────────────────────────────────────────────
function SimSlide1() {
  // Keşfet ekranı simülasyonu - tab highlight animasyonu
  ...
  useEffect(() => {
    ...
    // Keşfet'te (index 1) dur
    setTimeout(() => { clearInterval(t); setActiveTab(1); }, 2200);
    return () => clearInterval(t);
  }, []);
  const tabs = [{icon:"⚓",label:"Ana"},{icon:"🗺",label:"Keşfet"},{icon:"📊",label:"İstatistik"},{icon:"⚙️",label:"Ayarlar"}];
  return (
    <div className="ob-sim">
      <div className="ob-sim-header">
        <div className="ob-sim-dot" style={{background:"#ef5350"}}/>
        <div className="ob-sim-dot" style={{background:"#ffa726"}}/>
        <div className="ob-sim-dot" style={{background:"#66bb6a"}}/>
        <div className="ob-sim-title" style={{marginLeft:6}}>WORDVAULT</div>
      </div>
      <div className="ob-sim-body">
        <div className="ob-sim-tabs">
          {tabs.map((t,i) => (
            <div key={i} className={`ob-sim-tab ${activeTab===i?"active":""}`}>
              <div>{t.icon}</div>
              <div>{t.label}</div>
            </div>
          ))}
        ...
        <div style={{textAlign:"center",padding:"8px 0"}}>
          {activeTab===1 ? (
            <div style={{fontSize:11,color:"var(--g1)",fontFamily:"var(--fd)"}}>📚 Kelime Setleri</div>
          ) : (
            <div style={{fontSize:11,color:"var(--w3)"}}>...</div>
          )}
        </div>
        <div style={{textAlign:"center",marginTop:4}}>
          <div className="ob-sim-arrow">👇</div>
          <div style={{fontSize:10,color:"var(--g2)",marginTop:4}}>Keşfet sekmesine tıkla</div>
        </div>
      </div>
    </div>
  );
...
function SimSlide2() {
  ...
  useEffect(() => {
    ...
    const sets = [0,1,2];
    const t = setInterval(() => {
      setHighlighted(sets[i % sets.length]);
      i++;
    }, 600);
    setTimeout(() => { clearInterval(t); setHighlighted(1); }, 2000);
    return () => clearInterval(t);
  }, []);
  const sets = [
    {icon:"📘",name:"B1 Seviye",count:"120 kelime"},
    {icon:"📗",name:"B2 Seviye",count:"180 kelime"},
    {icon:"📕",name:"C1 Seviye",count:"240 kelime"},
  ];
  return (
    <div className="ob-sim">
      <div className="ob-sim-header">
        ...
        <div className="ob-sim-dot" style={{background:"#ffa726"}}/>
        <div className="ob-sim-dot" style={{background:"#66bb6a"}}/>
        <div className="ob-sim-title" style={{marginLeft:6}}>KEŞFEDİLEN SETLER</div>
      </div>
      <div className="ob-sim-body">
        <div className="ob-sim-sets">
          {sets.map((s,i) => (
            <div key={i} className={`ob-sim-set ${highlighted===i?"highlight":""}`}>
              <div className="ob-sim-set-icon">{s.icon}</div>
              <div>
                <div className="ob-sim-set-name">{s.name}</div>
                <div className="ob-sim-set-count">{s.count}</div>
              </div>
              {highlighted===i && <div style={{marginLeft:"auto",fontSize:10,color:"var(--g1)"}}>✓</div>}
            </div>
          ))}
        </div>
        <div style={{textAlign:"center",marginTop:10}}>
          <div className="ob-sim-arrow">👆</div>
          <div style={{fontSize:10,color:"var(--g2)",marginTop:2}}>Bir set seç</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="ob-sim">
      <div className="ob-sim-header">
        ...
        <div className="ob-sim-dot" style={{background:"#ffa726"}}/>
        <div className="ob-sim-dot" style={{background:"#66bb6a"}}/>
        <div className="ob-sim-title" style={{marginLeft:6}}>ANA EKRAN</div>
      </div>
      <div className="ob-sim-body">
        <div style={{fontSize:10,color:"var(--w4)",marginBottom:8,textAlign:"center"}}>Günlük hedef: 20 kelime</div>
        <div className={`ob-sim-start-btn ${pulse?"pulse":""}`}>▶ ÇALIŞMAYA BAŞLA</div>
        <div style={{display:"flex",gap:6,justifyContent:"center",marginTop:6}}>
          <div style={{padding:"5px 10px",borderRadius:8,background:"rgba(80,200,120,0.1)",border:"1px solid rgba(80,200,120,0.2)",fontSize:9,color:"#66bb6a"}}>📖 Çalış</div>
          <div style={{padding:"5px 10px",borderRadius:8,background:"rgba(212,175,55,0.1)",border:"1px solid rgba(212,175,55,0.2)",fontSize:9,color:"var(--g2)"}}>🔄 Tekrar</div>
          <div style={{padding:"5px 10px",borderRadius:8,background:"rgba(74,144,217,0.1)",border:"1px solid rgba(74,144,217,0.2)",fontSize:9,color:"#4A90D9"}}>⚡ Pratik</div>
        </div>
        <div style={{textAlign:"center",marginTop:8}}>
          <div className="ob-sim-arrow">👆</div>
          <div style={{fontSize:10,color:"var(--g2)",marginTop:2}}>Çalışmaya başla'ya bas</div>
        </div>
      </div>
    </div>
  );
...
function SimSlide4() {
  const [boxState, setBoxState] = useState([2,5,3,1,0]);
  useEffect(() => {
    const t = setInterval(() => {
      setBoxState(prev => {
        const next = [...prev];
        // Kutu 1'den kelime al, kutu 2'ye ekle simülasyonu
        if (next[0] > 0) { next[0]--; next[1]++; }
        else { next[0] = 3; next[1] = Math.max(0, next[1]-1); }
        return next;
      });
    }, 1200);
    return () => clearInterval(t);
  }, []);
  const boxColors = ["#ef5350","#ffa726","#4A90D9","#66bb6a","#9B59B6"];
  const boxLabels = ["Yeni","Günlük","Haftalık","Aylık","Bitti"];
  return (
    <div className="ob-sim">
      <div className="ob-sim-header">
        <div className="ob-sim-dot" style={{background:"#ef5350"}}/>
        <div className="ob-sim-dot" style={{background:"#ffa726"}}/>
        <div className="ob-sim-dot" style={{background:"#66bb6a"}}/>
        <div className="ob-sim-title" style={{marginLeft:6}}>LERTNEBİL SİSTEMİ</div>
      </div>
      <div className="ob-sim-body">
        <div style={{fontSize:9,color:"var(--w3)",textAlign:"center",marginBottom:10}}>Bildiğin kelimeler ilerler →</div>
        <div className="ob-sim-boxes">
          {boxState.map((count,i) => (
            <div key={i} className={`ob-sim-box ${count>0?"filled":""}`} style={count>0?{borderColor:boxColors[i]+"66",background:boxColors[i]+"11"}:{}}>
              <div className="ob-sim-box-num" style={{color:count>0?boxColors[i]:"var(--w2)"}}>{count}</div>
              <div style={{fontSize:7,color:"var(--w3)"}}>{boxLabels[i]}</div>
            </div>
          ))}
        </div>
        <div style={{textAlign:"center",marginTop:10,fontSize:10,color:"var(--w4)"}}>
          Bilmediğin kelimeler Kutu 1'e döner
        </div>
      </div>
    </div>
  );
...
const OB_STEPS = [
  {
    sim: <SimSlide1 />,
    title: "Keşfet'e Tıkla",
    desc: "Alt menüden 🗺 Keşfet sekmesine git ve kelime setlerini incele."
  },
  {
    sim: <SimSlide2 />,
    title: "Kelime Seti Seç",
    desc: "Seviyene uygun bir set seç. İstediğin kadar set ekleyebilirsin."
  },
  {
    sim: <SimSlide3 />,
    title: "Çalışmaya Başla",
    desc: "Ana ekrandan günlük oturumunu başlat. Çalış, Tekrar ve Pratik modları var."
  },
  {
    sim: <SimSlide4 />,
    title: "Sistemi Anla",
    desc: "Bildiğin kelimeler bir sonraki haftaya taşınır. Sadece zorlandıklarını tekrarlarsın!"
  },
];

// ── GİZLİLİK POLİTİKASI ONAY EKRANI ──────────────────────────────────────────
// Uygulama ilk kez açıldığında (cihaz bazlı, kullanıcı girişinden önce) gösterilir.
// Kabul edilmeden uygulamanın hiçbir ekranına geçilemez.
function PrivacyConsentScreen({ lang, onAccept }) {
  const [checked, setChecked] = useState(false);
  const isEn = lang === "en";

  return (
    <div className="screen" style={{display:"flex",flexDirection:"column",padding:"24px 20px",minHeight:"100%"}}>
      <div style={{flex:1}}>
        <div style={{fontSize:34,textAlign:"center",marginBottom:8}}>🔒</div>
        <div style={{fontFamily:"var(--fd)",fontSize:18,color:"var(--g2)",textAlign:"center",letterSpacing:1,marginBottom:18}}>
          {isEn ? "PRIVACY POLICY" : "GİZLİLİK POLİTİKASI"}
        </div>

        <div style={{background:"var(--n2)",borderRadius:"var(--r)",padding:16,fontSize:14,lineHeight:1.6,color:"var(--w9)",maxHeight:"48vh",overflowY:"auto",border:"1px solid rgba(212,175,55,.15)"}}>
          {isEn ? (
            <>
              <p>Welcome to Wordvault. Before you continue, please review how your data is handled:</p>
              <p><b>Local data:</b> Your account, learning progress, and study sessions are stored only on this device (in your browser's local storage), not on our servers.</p>
            ...
            <>
              <p>Wordvault'a hoş geldiniz. Devam etmeden önce verilerinizin nasıl işlendiğini lütfen okuyun:</p>
              <p><b>Yerel veriler:</b> Hesabınız, öğrenme ilerlemeniz ve çalışma oturumlarınız sadece bu cihazda (tarayıcının yerel hafızasında) saklanır, sunucularımıza gönderilmez.</p>
        ...
        <label style={{display:"flex",alignItems:"flex-start",gap:10,marginTop:18,fontSize:13,color:"var(--w7)",cursor:"pointer"}}>
        ...
        <div style={{textAlign:"center",marginTop:10}}>
          <a
            ...
            style={{color:"var(--g2)",fontSize:12,textDecoration:"underline"}}
          >
            {isEn ? "Read the full Privacy Policy" : "Gizlilik Politikasının tam metnini oku"}
          </a>
        </div>
      </div>
      ...
      <button
        ...
        style={{marginTop:18, opacity: checked ? 1 : .5}}
        disabled={!checked}
        onClick={() => { if (checked) onAccept(); }}
      >
        {isEn ? "I ACCEPT — CONTINUE" : "KABUL EDİYORUM — DEVAM ET"}
      </button>
    </div>
  );
}

function OnboardingScreen({ onDone, userId }) {
  const [step, setStep] = useState(0);
  const [noMore, setNoMore] = useState(false);
  const isLast = step === OB_STEPS.length - 1;
  const current = OB_STEPS[step];

  const finish = (forceSkip) => {
    if (noMore || forceSkip) localStorage.setItem("wv_ob_done_" + userId, "1");
    onDone();
  };
  ...
  return (
    <div className="ob-overlay">
      <div className="ob-header">
        <div className="ob-logo">⚓ WORDVAULT</div>
        <div className="ob-step-info">{step+1} / {OB_STEPS.length}</div>
      </div>

      <div className="ob-body">
        {current.sim}
        <div className="ob-caption">
          <div className="ob-caption-title">{current.title}</div>
          <div className="ob-caption-desc">{current.desc}</div>
        </div>
      </div>
      ...
      <div className="ob-footer">
        <div className="ob-dots">
          {OB_STEPS.map((_,i) => (
            <div key={i} className={`ob-dot ${i===step?"active":""}`} onClick={()=>setStep(i)} style={{cursor:"pointer"}} />
          ))}
        </div>
        ...
        <div className="ob-bottom">
          {!isLast
            ? <button className="ob-skip" onClick={()=>finish(true)}>Atla</button>
            : <div/>
          }
          {isLast && (
...
// ── YARDIMCI: kelimeyi düzgün formatlı göster (büyük harf sorununu önler) ─────
const fmt = (str) => {
  if (!str) return "";
  const lower = str.toLocaleLowerCase('en-US');
...
export default function App() {
  ...
  // Gizlilik politikası ilk açılışta onaylanmalı (cihaz bazlı, kullanıcıdan önce gelir)
  const [privacyAccepted, setPrivacyAccepted] = useState(
    () => (typeof window !== "undefined" ? localStorage.getItem("wv_privacy_accepted") === "1" : false)
  );
  ...
  // ── UYGULAMA BAŞLANGICI: IndexedDB başlat, önceki kullanıcıyı geri yükle ──
  // Not: Bu adım tamamen cihaz üzerinde (IndexedDB + gömülü veri) çalışır,
  // internet gerektirmez. Uygulama internetsiz de anında açılır.
  useEffect(() => {
    const boot = async () => {
      try {
        // Gizlilik politikası onaylanmadıysa ağ isteği yapılmaz (sadece gömülü veri kullanılır)
        const wordData = await fetchWordData(privacyAccepted);
        await initDB(wordData);
        const savedUid = localStorage.getItem("wv_uid");
        if (savedUid) {
          const u = await dGet("users", parseInt(savedUid, 10));
          // Admin kullanıcılar her açılışta şifre girmek zorunda
          if (u && !u.isAdmin) setUser(u);
          else localStorage.removeItem("wv_uid");
        }
      } catch (e) {
  ...
  // ── ARKA PLAN GÜNCELLEME: internet varsa Supabase'deki içeriği çek ve
  // cihaza (IndexedDB) yaz; yoksa sessizce geç. Uygulama açılışını bekletmez.
  // Gizlilik politikası onaylanmadan herhangi bir ağ isteği yapılmaz. ──
  useEffect(() => {
    if (!ready || !privacyAccepted) return;
    let cancelled = false;
    (async () => {
      try {
        const [lvlsRes, setsRes, wordsRes] = await Promise.all([
          supabase.from("levels").select("*").order("orderIndex", { ascending: true }),
          supabase.from("wordsets").select("*").order("orderIndex", { ascending: true }),
          supabase.from("words").select("*").limit(10000),
        ]);
        if (cancelled) return;
        ...
        if (lvls.length > 0 && words.length > 0) {
          await syncContentToDB(lvls, sets, words);
        }
      } catch (e) {
        // İnternet yok veya erişilemedi — sorun değil, cihazdaki veriyle devam edilir.
        console.warn("Arka plan içerik güncellemesi yapılamadı (offline olabilir):", e.message);
      }
    })();
    return () => { cancelled = true; };
  }, [ready, privacyAccepted]);

  // ── TAM EKRAN: ilk dokunuşta otomatik tam ekrana geç ────────────────────────
  useEffect(() => {
    const goFs = () => {
  ...
  const handleLogin = (u) => {
    ...
    // Her kullanıcı için ayrı onboarding kontrolü
    const obKey = "wv_ob_done_" + u.id;
    if (!localStorage.getItem(obKey)) setShowOb(true);
  };
  ...
  if (!ready) return <div className="app"><style>{CSS}</style><button className="fs-btn" onClick={toggleFs}>{fs?"⊡":"⛶"}</button><div className="loading-screen">⚓</div></div>;

  if (!privacyAccepted) return (
    <div className="app">
      <style>{CSS}</style>
      <button className="fs-btn" onClick={toggleFs}>{fs?"⊡":"⛶"}</button>
  ...
  if (showOb) return (
    <div className="app">
      <style>{CSS}</style>
      <OnboardingScreen userId={user.id} onDone={() => setShowOb(false)} />
    </div>
  );
...
function AuthScreen({ onLogin }) {
  ...
  const adminLogin = async () => {
    try {
      const ok = await adminVerifyPassword(adminPass);
      if (!ok) { setError(lang==="en"?"Incorrect password.":"Şifre hatalı."); return; }
      const admins = users.filter(u=>u.isAdmin);
      if (!admins.length) { setError("Admin kullanıcı bulunamadı."); return; }
      onLogin(admins[0]);
  ...
  const handleUserLogin = async () => {
    const h = await hashPassword(userPass);
    if (selectedUser.passwordHash === h) { onLogin(selectedUser); }
    else { setError(lang==="en"?"Incorrect password.":"Şifre hatalı."); }
  };
  ...
  if (mode==="userPass" && selectedUser) return (
    <div className="auth-bg">
      ...
      <div className="auth-form" style={{width:"100%",maxWidth:320}}>
        <div className="field"><label>{lang==="en"?"PASSWORD":"ŞİFRE"}</label><input type="password" placeholder="••••••" value={userPass} onChange={e=>setUserPass(e.target.value)} onKeyDown={e=>{if(e.key==="Enter") handleUserLogin();}} autoFocus /></div>
        {error && <div className="auth-err">{error}</div>}
        <button className="btn btn-gold" onClick={handleUserLogin}>{lang==="en"?"LOGIN":"GİRİŞ"}</button>
...
function ExploreScreen({ user, lang, showToast }) {
  ...
  const applyData = useCallback((lvls, sets, words, acts) => {
    setLevels(lvls);
    ...
    setWCount(wc);
    const wm={}; words.forEach(w=>{if(!wm[w.wordSetId])wm[w.wordSetId]=[];wm[w.wordSetId].push(w);});
    setWordsMap(wm);
    setAllWords(words);
  ...
  const load = useCallback(async () => {
    // 1) Önce IndexedDB'den anında göster (cache-first)
    const [cachedLvls, cachedSets, cachedWords, acts] = await Promise.all([
      dAll("levels"),
      dAll("wordsets"),
      dAll("words"),
      dIdx("active_sets", "userId", user.id),
    ]);
    if (cachedLvls.length > 0) {
      applyData(cachedLvls, cachedSets, cachedWords, acts);
    }

    // 2) İnternet varsa Supabase'den güncel içeriği çek, cihaza (IndexedDB) kaydet
    try {
      const [lvlsRes, setsRes, wordsRes] = await Promise.all([
        supabase.from("levels").select("*").order("orderIndex", { ascending: true }),
        supabase.from("wordsets").select("*").order("orderIndex", { ascending: true }),
      ...
      if (lvls.length > 0 && words.length > 0) {
        await syncContentToDB(lvls, sets, words);
        const freshActs = await dIdx("active_sets", "userId", user.id);
        applyData(lvls, sets, words, freshActs);
      }
    } catch (e) {
      // İnternet yok veya istek başarısız — sorun değil, IndexedDB'deki (cihazdaki) veriyle devam edilir.
      console.warn("Explore sync hatası (offline olabilir):", e.message);
    }
  }, [user.id, applyData]);
  ...
  const toggle = async (setId, words_count) => {
    ...
    // Sadece active_sets güncelle, Supabase'e gitme
    const freshActs = await dIdx("active_sets", "userId", user.id);
    setActIds(new Set(freshActs.map(a=>a.wordSetId)));
  };
...
function WordCard({ word, lang, compact }) {
  ...
  if (compact) return (
    <div onClick={()=>setOpen(o=>!o)} style={{padding:"6px 4px",cursor:"pointer",borderRadius:8,transition:"background .15s"}} >
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <div style={{flex:1}}>
          <span style={{fontSize:13,color:"var(--w9)",fontWeight:600}}>{fmt(word.english)}</span>
          <span style={{fontSize:12,color:"var(--w4)",marginLeft:6}}>{fmt(word.turkish)}</span>
        </div>
  ...
  return (
    <div className="word-item" onClick={()=>setOpen(o=>!o)} style={{cursor:"pointer"}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <div style={{flex:1}}>
          <div className="word-item-en">{fmt(word.english)}</div>
          <div className="word-item-tr">{fmt(word.turkish)}</div>
        </div>
  ...
  return (
    <div style={{marginTop:8,paddingTop:8,borderTop:"1px solid rgba(212,175,55,0.12)"}}>
      ...
      {word.synonyms && <>
        ...
        <div style={{fontSize:9,color:"#4A90D9",fontFamily:"var(--fd)",letterSpacing:1,marginBottom:2}}>🔤 EŞ ANLAMLILARI</div>
        <div style={{fontSize:12,color:"var(--w7)",lineHeight:1.5}}>{word.synonyms}</div>
      </>}
      {!word.ydsExampleSentence && !word.mnemonicTip && !word.synonyms && <div style={{fontSize:12,color:"var(--w4)"}}>{lang==="en"?"No extra info.":"Ek bilgi yok."}</div>}
    </div>
  );
}
      ...
      <div className="fc-wrap">
        ...
        <div key={cardKey} className={`fc ${flipped?"flipped":""}`} onClick={()=>setFlipped(f=>!f)}>
          <div className="fc-face fc-front">
            ...
            <div className="fc-word">{fmt(word.english)}</div>
            <div className="fc-hint">{lang==="en"?"Tap to flip":"Çevirmek için dokun"}</div>
          </div>
          <div className="fc-face fc-back">
            <div className="fc-en">{fmt(word.english)}</div>
            <div className="fc-tr">{fmt(word.turkish)}</div>
            {word.ydsExampleSentence && <>
            ...
            {word.synonyms && <>
              <div className="fc-div"/>
              <div className="fc-stitle" style={{color:"#4A90D9"}}>🔤 EŞ ANLAMLILARI</div>
              <div className="fc-sbody">{word.synonyms}</div>
            </>}
          </div>
        </div>
      </div>
...
function SettingsScreen({ user, lang, onLangChange, onUserUpdate, onLogout, onSwitchUser, showToast }) {
  ...
  const [userPwOld,  setUserPwOld]  = useState("");
  ...
  const [removePwOld,  setRemovePwOld]  = useState("");
  const [removePwErr,  setRemovePwErr]  = useState("");
  // Delete account
  ...
  const saveAdminPw = async () => {
    ...
    try {
      const { newHash } = await adminChangePassword(adminOld, adminNew);
    ...
    } catch(e) { setPwErr("Kayıt hatası: " + e.message); }
  };

  const saveUserPw = async () => {
    setUserPwErr("");
    // Eğer kullanıcının zaten şifresi varsa, eski şifreyi doğrula
    if (user.passwordHash) {
      if (!userPwOld) { setUserPwErr(lang==="en"?"Enter your current password.":"Mevcut şifreni gir."); return; }
      const oldHash = await hashPassword(userPwOld);
      if (oldHash !== user.passwordHash) { setUserPwErr(lang==="en"?"Current password is incorrect.":"Mevcut şifre hatalı."); return; }
    }
    ...
    const updated = {...user,passwordHash:await hashPassword(userPwNew)};
    await dPut("users",updated);
    onUserUpdate(updated);
    setUserPwOld("");setUserPwNew("");setUserPwNew2("");setShowSetPw(false);
    showToast(lang==="en"?"Password updated ✓":"Şifre güncellendi ✓");
  };

  const removeUserPw = async () => {
    setRemovePwErr("");
    if (!removePwOld) { setRemovePwErr(lang==="en"?"Enter your current password.":"Mevcut şifreni gir."); return; }
    const oldHash = await hashPassword(removePwOld);
    if (oldHash !== user.passwordHash) { setRemovePwErr(lang==="en"?"Current password is incorrect.":"Mevcut şifre hatalı."); return; }
    const updated = {...user, passwordHash:""};
    await dPut("users",updated);
    onUserUpdate(updated);
    setRemovePwOld("");setShowRemovePw(false);
    showToast(lang==="en"?"Password removed ✓":"Şifre kaldırıldı ✓");
  };

  const deleteOwnAccount = async () => {
    setDeleteAccErr("");
    if (user.passwordHash) {
      if (!deleteAccPw) { setDeleteAccErr(lang==="en"?"Enter your password to confirm.":"Onaylamak için şifreni gir."); return; }
      const oldHash = await hashPassword(deleteAccPw);
      if (oldHash !== user.passwordHash) { setDeleteAccErr(lang==="en"?"Password is incorrect.":"Şifre hatalı."); return; }
    }
    // Kullanıcıya ait tüm verileri temizle
    try {
      const [prog, sessions, acts] = await Promise.all([
        dIdx("progress","userId",user.id),
        dIdx("sessions","userId",user.id),
        dIdx("active_sets","userId",user.id)
      ]);
      for (const p of prog)     await dDel("progress",[p.userId,p.wordId]);
      for (const s of sessions) await dDel("sessions",s.id);
      for (const a of acts)     await dDel("active_sets",[a.userId,a.wordSetId]);
    } catch(e) { console.error("Hesap verisi temizleme hatası:", e); }
    await dDel("users", user.id);
    localStorage.removeItem("wv_uid");
    showToast(lang==="en"?"Account deleted":"Hesap silindi");
    onLogout();
  };
  ...
  return (
    <>
      ...
      <div className="settings-wrap">
        ...
        {!user.isAdmin && (
          <div className="scard">
            <div className="srow" style={{marginBottom:showSetPw?10:0}}>
              ...
              <button style={{background:"none",border:"1px solid rgba(212,175,55,.3)",borderRadius:8,padding:"4px 10px",color:"var(--g2)",fontFamily:"var(--fd)",fontSize:10,letterSpacing:.5,cursor:"pointer"}} onClick={()=>{setShowSetPw(s=>!s);setShowRemovePw(false);}}>
                {showSetPw?"✕":(user.passwordHash?(lang==="en"?"Change":"Değiştir"):(lang==="en"?"Set":"Oluştur"))}
              </button>
            </div>
            {showSetPw && <>
              {user.passwordHash && (
                <input className="mi" type="password" placeholder={lang==="en"?"Current password":"Mevcut şifre"} value={userPwOld} onChange={e=>setUserPwOld(e.target.value)}/>
              )}
            ...
            {/* Şifre kaldır - sadece şifresi varsa göster */}
            {user.passwordHash && !showSetPw && (
              <>
                <div className="srow" style={{marginTop:10,marginBottom:showRemovePw?10:0}}>
                  <span className="s-icon">🔓</span>
                  <div style={{flex:1}}>
                    <div className="s-label">{lang==="en"?"Remove Password":"Şifreyi Kaldır"}</div>
                    <div className="s-sub">{lang==="en"?"Login without password":"Şifresiz giriş yap"}</div>
                  </div>
                  <button style={{background:"none",border:"1px solid rgba(229,57,53,.3)",borderRadius:8,padding:"4px 10px",color:"#ef5350",fontFamily:"var(--fd)",fontSize:10,letterSpacing:.5,cursor:"pointer"}} onClick={()=>setShowRemovePw(s=>!s)}>
                    {showRemovePw?"✕":(lang==="en"?"Remove":"Kaldır")}
                  </button>
                </div>
                {showRemovePw && <>
                  <input className="mi" type="password" placeholder={lang==="en"?"Current password":"Mevcut şifre"} value={removePwOld} onChange={e=>setRemovePwOld(e.target.value)}/>
                  {removePwErr&&<div className="auth-err">{removePwErr}</div>}
                  <button className="btn btn-red" onClick={removeUserPw}>{lang==="en"?"REMOVE PASSWORD":"ŞİFREYİ KALDIR"}</button>
                </>}
              </>
            )}
          </div>
        ...
        {/* Hesabı Sil - admin olmayan kullanıcılar için */}
        {!user.isAdmin && (
          <div className="scard">
            <div className="srow" style={{marginBottom:showDeleteAcc?10:0}}>
              <span className="s-icon">⚠️</span>
              <div style={{flex:1}}>
                <div className="s-label" style={{color:"#ef5350"}}>{lang==="en"?"Delete Account":"Hesabı Sil"}</div>
                <div className="s-sub">{lang==="en"?"This cannot be undone":"Bu işlem geri alınamaz"}</div>
              </div>
              <button style={{background:"none",border:"1px solid rgba(229,57,53,.3)",borderRadius:8,padding:"4px 10px",color:"#ef5350",fontFamily:"var(--fd)",fontSize:10,letterSpacing:.5,cursor:"pointer"}} onClick={()=>setShowDeleteAcc(s=>!s)}>
                {showDeleteAcc?"✕":(lang==="en"?"Delete":"Sil")}
              </button>
            </div>
            {showDeleteAcc && <>
              <div style={{fontSize:12,color:"var(--w5)",marginBottom:10,lineHeight:1.6}}>
                {lang==="en"
                  ?"All your progress, sessions, and stats will be permanently deleted."
                  :"Tüm ilerlemen, oturumların ve istatistiklerin kalıcı olarak silinecek."}
              </div>
              {user.passwordHash && (
                <input className="mi" type="password" placeholder={lang==="en"?"Enter your password to confirm":"Onaylamak için şifreni gir"} value={deleteAccPw} onChange={e=>setDeleteAccPw(e.target.value)}/>
              )}
              {deleteAccErr&&<div className="auth-err">{deleteAccErr}</div>}
              <button className="btn btn-red" onClick={deleteOwnAccount}>{lang==="en"?"PERMANENTLY DELETE ACCOUNT":"HESABI KALICI OLARAK SİL"}</button>
            </>}
          </div>
        )}
        ...
        {/* Designed by */}
        <div style={{textAlign:"center",color:"var(--w3)",fontSize:11,fontFamily:"var(--fd)",letterSpacing:"0.5px",paddingBottom:6}}>
          Designed by Ömer BADİŞ
        </div>
...
function AdminScreen({ user, lang, showToast }) {
  ...
  const loadLevels = async () => {
    const { data } = await supabase.from("levels").select("*").order("orderIndex", { ascending: true });
    setLevels(data || []);
  };
  const loadSets = async (lid) => {
    const { data } = await supabase.from("wordsets").select("*").eq("levelId", lid).order("orderIndex", { ascending: true });
    setSets(data || []);
  };
  const loadWords  = async (sid) => {
    ...
    if (error) { showToast("Kelime yükleme hatası: " + error.message); console.error("loadWords error:", error); return; }
    setWords(data || []);
  };
  const loadAllWords = async () => {
    const { data, error } = await supabase.from("words").select("*").limit(10000);
    if (error) { console.error("loadAllWords error:", error); return; }
    setAllWords(data || []);
  };
  ...
  if(selSet) loadWords(selSet.id);
          ...
          {!selLevel && <>
            ...
            {levels.map(level=>(
              <div key={level.id} className="admin-item" onClick={()=>{setSelLevel(level);loadSets(level.id);}}>
                ...
                <button className="del-btn" onClick={async e=>{e.stopPropagation();await adminDeleteLevel(level.id);loadLevels();showToast(lang==="en"?"Deleted":"Silindi");}}>🗑</button>
              </div>
            ))}
          </>}
          ...
          {selLevel&&!selSet&&<>
            ...
            {sets.map(set=>(
              <div key={set.id} className="admin-item" onClick={()=>{setSelSet(set);loadWords(set.id);}}>
                ...
                <span style={{color:"var(--w4)",fontSize:17}}>›</span>
                <button className="del-btn" onClick={async e=>{e.stopPropagation();await adminDeleteWordset(set.id);loadSets(selLevel.id);showToast(lang==="en"?"Deleted":"Silindi");}}>🗑</button>
              </div>
            ))}
          </>}
          ...
          {selSet&&<>
            ...
            {words.map(word=>(
              <div key={word.id} className="word-item" style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                ...
                <button style={{background:"none",border:"none",color:"var(--g2)",fontSize:15,cursor:"pointer",padding:"2px 5px"}} onClick={()=>{setEditWord(word);setModal("editWord");}}>✏️</button>
                <button className="del-btn" onClick={async()=>{await adminDeleteWord(word.id);loadWords(selSet.id);}}>🗑</button>
              </div>
            ))}
          </>}
        </>}
      ...
      {/* MODALS */}
      {modal==="level"&&<CreateLevelModal lang={lang} onClose={()=>setModal(null)} onSave={async d=>{await adminInsertLevel({...d,orderIndex:levels.length,createdAt:Date.now()});loadLevels();setModal(null);showToast(lang==="en"?"Level created!":"Seviye oluşturuldu!");}}/>}
      {modal==="set"&&selLevel&&<CreateSetModal lang={lang} levelName={selLevel.name} onClose={()=>setModal(null)} onSave={async d=>{await adminInsertWordset({...d,levelId:selLevel.id,orderIndex:sets.length,createdAt:Date.now()});loadSets(selLevel.id);setModal(null);showToast(lang==="en"?"Set created!":"Set oluşturuldu!");}}/>}
      {modal==="word"&&selSet&&<AddWordModal lang={lang} setId={selSet.id} orderIndex={words.length} onClose={()=>setModal(null)} onSaved={()=>{loadWords(selSet.id);loadAllWords();showToast(lang==="en"?"Word added!":"Kelime eklendi!");}} checkDuplicate={async(eng)=>{const {data}=await supabase.from("words").select("english").eq("wordSetId",selSet.id);return (data||[]).some(w=>w.english.toLowerCase()===eng.toLowerCase());}}/>}
...
function AddWordModal({lang,setId,orderIndex,onClose,onSaved,checkDuplicate}){
  const [f,setF]=useState({english:"",turkish:"",ydsExampleSentence:"",ydsExampleTranslation:"",mnemonicTip:"",synonyms:""});
  ...
  const save=async()=>{
    ...
    try {
      await adminInsertWord({ ...f, wordSetId: setId, orderIndex, addedAt: Date.now() });
      onSaved();
      setF({english:"",turkish:"",ydsExampleSentence:"",ydsExampleTranslation:"",mnemonicTip:"",synonyms:""});
      setErr("");
    } catch(e) { setErr("Kayıt hatası: " + e.message); }
  };
  return <Modal lang={lang} title={lang==="en"?"ADD WORD":"KELİME EKLE"} onClose={onClose} onConfirm={save} confirmLabel={lang==="en"?"Save":"Kaydet"}>
    ...
    <textarea className="mi" placeholder="Mnemonic (isteğe bağlı)" value={f.mnemonicTip} onChange={e=>setF({...f,mnemonicTip:e.target.value})}/>
    <textarea className="mi" placeholder={lang==="en"?"Synonyms (e.g: big, huge, enormous)":"Eş anlamlılar (örn: big, huge, enormous)"} value={f.synonyms||""} onChange={e=>setF({...f,synonyms:e.target.value})}/>
    {err&&<div className="auth-err">{err}</div>}
  </Modal>;
}

function EditWordModal({lang,word,onClose,onSaved}){
  const [f,setF]=useState({english:word.english,turkish:word.turkish,ydsExampleSentence:word.ydsExampleSentence||"",ydsExampleTranslation:word.ydsExampleTranslation||"",mnemonicTip:word.mnemonicTip||"",synonyms:word.synonyms||""});
  const save = async () => {
    await adminUpdateWord(word.id, f);
    onSaved();
  };
...
function FullscreenBtn({lang}) {
  ...
  return (
    <div className="srow" onClick={toggle} style={{cursor:"pointer"}}>
      <span className="s-icon">⛶</span>
      <div style={{flex:1}}>
        <div className="s-label">{fs ? (lang==="en"?"Exit Fullscreen":"Tam Ekrandan Çık") : (lang==="en"?"Fullscreen":"Tam Ekran")}</div>
        <div className="s-sub">{fs ? (lang==="en"?"Restore window":"Pencereye dön") : (lang==="en"?"Fill the screen":"Ekranı doldur")}</div>
      </div>
      <span style={{fontSize:18,opacity:.5}}>{fs ? "🔲" : "⤢"}</span>
    </div>
  );
}
...
function ImportModal({lang,set,onClose,onDone,showToast}) {
    ...
    try {
      ...
      if (isJson) {
        ...
        rows = arr.map(w => ({
          ...
          english: w.english || "",
          turkish: w.turkish || "",
          ydsExampleSentence: w.ydsExample || w.ydsExampleSentence || "",
          ydsExampleTranslation: w.ydsTranslation || w.ydsExampleTranslation || "",
          mnemonicTip: w.mnemonic || w.mnemonicTip || "",
          orderIndex: 0,
          addedAt: Date.now()
        }));
      } else {
        // CSV: english,turkish,ydsExample,ydsTranslation,mnemonic
        const lines = text.trim().split("\n");
        const start = lines[0].toLowerCase().startsWith("english") ? 1 : 0;
        rows = lines.slice(start).filter(l => l.trim()).map((l, i) => {
          const [en, tr, yds, ydsT, mn] = l.split(",").map(c => c.trim().replace(/^"|"$/g, ""));
          return {
            wordSetId: set.id,
            english: en || "",
            turkish: tr || "",
            ...
            mnemonicTip: mn || "",
            synonyms: cols[5]?.trim() || "",
            orderIndex: i,
            addedAt: Date.now()
          };
        });
      }
      const validRows = rows.filter(w => w.english && w.turkish);
      const skipped   = rows.length - validRows.length;
      if (!validRows.length) { showToast(lang==="en"?"No valid rows found.":"Geçerli satır bulunamadı."); return; }
      await adminInsertWords(validRows);
      setResult({ added: validRows.length, skipped });
      onDone();
    } catch (e) {
      showToast((lang==="en"?"Error: ":"Hata: ") + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFile = (e) => {
    ...
    reader.onload = async (ev) => {
      await doImport(ev.target.result, file.name.endsWith(".json"));
    };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        <div className="modal-title">📁 {lang==="en"?"IMPORT WORDS":"KELİME AKTAR"}</div>
        <div style={{color:"var(--w6)",fontSize:12,marginBottom:12,lineHeight:1.6}}>
          {lang==="en"
            ? "CSV format: english, turkish, ydsExample, ydsTranslation, mnemonic, synonyms"
            : "CSV formatı: english, turkish, ydsExample, ydsTranslation, mnemonic, synonyms"}
        </div>

        {result ? (
          <div style={{textAlign:"center",padding:"12px 0"}}>
            <div style={{fontSize:28,marginBottom:8}}>✅</div>
            <div style={{color:"var(--g2)",fontSize:16,fontWeight:700}}>{result.added} {lang==="en"?"words added":"kelime eklendi"}</div>
            {result.skipped>0 && <div style={{color:"var(--w4)",fontSize:12,marginTop:4}}>{result.skipped} {lang==="en"?"skipped (missing english/turkish)":"atlandı (İngilizce/Türkçe eksik)"}</div>}
          </div>
        ) : (
          <>
            <input
            ...
            >
              {loading
                ...
                : (lang==="en"?"📂 Choose File (CSV or JSON)":"📂 Dosya Seç (CSV veya JSON)")}
            </button>
          </>
        )}

        <div className="modal-btns" style={{marginTop:8}}>
          <button className="btn btn-outline" onClick={onClose}>{lang==="en"?"Close":"Kapat"}</button>
        </div>
      </div>
    </div>
  );
