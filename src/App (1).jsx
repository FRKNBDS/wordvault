...
async function getStudyWords(userId, dailyGoal, sessionType) {
  ...
  const { data: _allW } = await supabase.from("words").select("*");
  const allWords = _allW || [];
  ...
  const { data: wordRows } = await supabase.from("words").select("*").in("id", wordIds);
  const wordById = {}; (wordRows||[]).forEach(w=>{wordById[w.id]=w;});
...
async function getPracticeWords(userId, setIds) {
  const { data } = await supabase.from("words").select("*").in("wordSetId", setIds);
...
async function activateSet(userId, wordSetId) {
  ...
  const { data: words } = await supabase.from("words").select("id").eq("wordSetId", wordSetId);
...
async function deactivateSet(userId, wordSetId) {
  ...
  if (otherActiveSetIds.length) {
    const { data: ws } = await supabase.from("words").select("id").in("wordSetId", otherActiveSetIds);
...
function ExploreScreen({ user, lang, showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    // 2) Arka planda Supabase'den kontrol et — sadece yeni veri varsa güncelle
    try {
      const lastSync = parseInt(localStorage.getItem("wv_explore_sync") || "0", 10);
      const now = Date.now();
      const SYNC_INTERVAL = 5 * 1000; // 5 saniyede bir kontrol et
      ...
      if (now - lastSync > SYNC_INTERVAL) {
        ...
        if (lvls.length > 0) {
          const dbLvls  = await dAll("levels");
          const dbSets  = await dAll("wordsets");
          const dbWords = await dAll("words");

          const changed =
            lvls.length  !== dbLvls.length  ||
            sets.length  !== dbSets.length  ||
            words.length !== dbWords.length;

          if (changed) {
            for (const l of lvls)  await dPut("levels",   l);
            for (const s of sets)  await dPut("wordsets", s);
            for (const w of words) await dPut("words",    w);
            const freshActs = await dIdx("active_sets", "userId", user.id);
            applyData(lvls, sets, words, freshActs);
          }
          localStorage.setItem("wv_explore_sync", String(now));
        }
      }
    } catch (e) {
      console.warn("Explore sync hatası:", e.message);
    }
  }, [user.id, applyData]);
